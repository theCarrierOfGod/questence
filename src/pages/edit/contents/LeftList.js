...
const LeftList = () => {
    ...
    return (
        <>
            <div className='conMenu p-0 w-100 mb-5 mt-3'>
                                                    ...
                                                    <ul className={`${(detail.activeSub === subSection.id) ? 'd-block' : 'd-none'} no-list-style pl-20`} >
                                                        {(subSection.lessons.map((lesson) => (
                                                            <li id={lesson.id} key={lesson.id} style={{ padding: '0' }}>
                                                                ...
                                                                <span className={`${(detail.activeLesson === lesson.id) ? 'activeTab' : ''} w-100 d-flex alignCenter p-1`} onClick={e => detail.toggleActiveLesson(lesson.id, lesson)}>
                                                                    <span>
                                                                        {lesson.title}
                                                                    </span>
                                                                </span>
